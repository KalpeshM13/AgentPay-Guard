...
export async function connectWallet() {
  ...
  // Force switch to Hardhat Local network (Chain ID 31337 -> 0x7a69)
  try {
...
export async function updateLimits(perTxEth, dailyEth) {
  const { signer } = await connectWallet();
  const contract = await getContract(signer);
  const tx = await contract.setLimits(ethers.parseEther(perTxEth.toString()), ethers.parseEther(dailyEth.toString()));
  await tx.wait();
  return tx.hash;
}
